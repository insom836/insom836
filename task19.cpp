...
void UpdateGame()
{
    ...
    if (inMenu)
    {
        ...
        if (IsKeyPressed(KEY_C)) 
        {
            ...
            lives = 3;
            currentLevel = 0;
            std::cout << "  [操作] 玩家已一键清除所有存档数据。" << std::endl;
        }
...
void DrawGame()
{
    ...
    if (inMenu)
    {
        ...
        if (CheckSaveExists()) 
        {
            ...
            // 提示清除存档的操作
            DrawText("PRESS C TO CLEAR SAVE", SCREEN_WIDTH / 2 - 110, SCREEN_HEIGHT - 30, 20, Fade(GRAY, 0.7f));
        }
...
int main()
{
    ...
    // 就在下面这行，改成了你要的标题
    InitWindow(SCREEN_WIDTH, SCREEN_HEIGHT, "Li Yuhan and Liang Zhixia Breakout: Dual Core (Max Edition)");
    InitAudioDevice();
