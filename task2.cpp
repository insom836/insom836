...
// 挡板初始长度变为原来的 1.7 倍 (120 * 1.7 = 204)
const int INITIAL_PADDLE_WIDTH = 204;
const int PADDLE_HEIGHT = 20;
// 挡板最大长度限制为屏幕宽度的 70% (800 * 0.7 = 560)
const float MAX_PADDLE_WIDTH = SCREEN_WIDTH * 0.7f; 
    ...
    for (int i = 0; i < (int)balls.size(); i++)
    {
        ...
        for (auto &b : bricks)
        {
            if (b.active && CheckCollisionCircleRec(balls[i].pos, balls[i].radius, b.rec))
            {
                ...
                if (bricksHit == 5 && !secondBallTriggered)
                {
                    secondBallTriggered = true;
                    Ball b2 = {{SCREEN_WIDTH / 2.0f, SCREEN_HEIGHT / 2.0f}, {-4.0f, -4.0f}, (float)BALL_RADIUS, true};
                    balls.push_back(b2);

                    ripplePos = b2.pos;
                    rippleRadius = 0;
                    rippleAlpha = 1.0f;
                    TriggerShake(0.5f, 10.0f);
                    PlaySound(secondBallFx);
                }
...
void DrawGame()
{
    ...
    else
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            float heartPulse = (i < lives) ? sinf(GetTime() * 4.0f + i) * 1.5f : 0;
            DrawUIHeart(SCREEN_WIDTH - 180 + i * 25, 35, 15 + heartPulse, heartColor);
        }
